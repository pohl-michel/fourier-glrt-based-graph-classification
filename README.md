...
The data in "subjects_data.mat" contains statistical features for each brain region in segmentated brain FDG-PET images. Those 142 images were provided by "La Timone" Hospital (Marseille) and correspond to 61 healthy control subjects and 81 patients with Alzheimer's disease. Each brain image was segmented into 116 regions of interest (ROIs) using the WFU-Pickatlas tool. For each region, the first four moments (mean, variance, skewness, and kurtosis) and the entropy are computed, leading to two tensors, each of size 116 * 5 * *N*, where *N* denotes the number of healthy subjects or Alzheimer's patients. 
